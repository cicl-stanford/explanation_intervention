          ...
          document.querySelector("form").addEventListener("submit", (event) => {
              ...
              for (var i = 0; i < trial.questions.length; i++) {
                  ...
                  if (match.querySelector("input[type=radio]:checked") !== null) {
                      val_raw = match.querySelector("input[type=radio]:checked").value;
                  }
                  else {
                      val_raw = "";
                  }
                  ...
                  if (match.attributes["data-name"].value !== "") {
                      ...
                      // if the question is a selection task, reformat the way that the data is saved
                      const selection_substr = "selection_task";
                      const intervention_substr = "intervention_task";
                      if (name.toLowerCase().includes(selection_substr) || name.toLowerCase().includes(intervention_substr)) {
                        val_raw = match.querySelector("input[type=radio]:checked").value;
                        option_number = match.querySelector("input[type=radio]:checked").id;
                        option_number = option_number.charAt(option_number.length-1);
                        if (option_number === '0') {
                            val_clean = 'blue';
                        } else if (option_number === '1') {
                            val_clean = 'orange';
                        } else if (option_number === '2') {
                            val_clean = 'no preference';
                        }
                        obje[name] = { val_raw, val_clean };
                      } else {  // if not, keep the data as is
                        obje[name] = val_raw;
                      }
                  }
