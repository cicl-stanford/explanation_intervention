...
const cp_instructions_comp_long = {
    ...
    on_finish: function() {
        var prog = jsPsych.getProgressBarCompleted();
        jsPsych.setProgressBar(prog + 0.25); 
    }
};
...
const cp_selection_task_long = {
    ...
    preamble: `<div style="text-align:center; padding:20px; max-width:900px;">` +
        ...
        Zarah gets to turn one of the switches <strong>ON</strong> to try to win money for your team. \
...
const cp_intervention_task_long = {
    type: jsPsychSurveyMultiChoice,
    preamble: `<div style="text-align:center; padding:20px; max-width:900px;">` +
        ...
        "<img class='cp_image' src='media/cp_long/intervention_task.jpg'></img>",
    questions: [
        {
            name: 'cp_intervention_task',
            prompt: trialData["conjunctive_positive"]["intervention_task"]["question"],
            options: trialData["conjunctive_positive"]["intervention_task"]["options"],
            required: true,
        },
    ],
    // on_start: function() {
...
let cp_arr_long2 = [];
let cp_len_long2 = trialData["conjunctive_positive"]["instructions_long2"].length;
for (let i = 0; i < cp_len_long2; i++) {
    cp_arr_long2.push(
        trialData["conjunctive_positive"]["instructions_long2"][i]
    );
}

var instruction_pages_long2 = [];
for (var i = 0; i < cp_len_long2; i++) {
    let slide = '<div><img class="cp_image" src="media/cp_long/setup2_' + i + '.jpg"></img>' +
        '<p>' + cp_arr_long2[i] + '</p></div>';
    instruction_pages_long2.push(slide);
}

const cp_instructions_long2 = {
    type: jsPsychInstructions,
...
const cp_long = [
    ...
    cp_instructions_long2,
    cp_intervention_task_long,
    cp_round_one_long,
    cp_round_one_comp_q1_long,
