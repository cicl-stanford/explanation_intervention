...
const cn_instructions_comp_long = {
    ...
    on_finish: function() {
        var prog = jsPsych.getProgressBarCompleted();
        jsPsych.setProgressBar(prog + 0.375); 
    }
};
...
const cn_round_two_long = {
    ...
    on_finish: function () {
        var prog = jsPsych.getProgressBarCompleted();
        jsPsych.setProgressBar(prog + 0.875); 
    }
};
...
const cn_selection_task_long = {
    ...
    preamble: `<div style="text-align:center; padding:20px; max-width:900px;">` +
        ...
        you can explain why your team won $5 the last round.<br></br>" +
...
let cn_arr_long2 = [];
let cn_len_long2 = trialData["conjunctive_negative"]["instructions_long2"].length;
for (let i = 0; i < cn_len_long2; i++) {
    cn_arr_long2.push(
        trialData["conjunctive_negative"]["instructions_long2"][i]
    );
}

var instruction_pages_long2 = [];
for (var i = 0; i < cn_len_long2; i++) {
    let slide = '<div><img class="cn_image" src="media/cn_long/setup2_' + i + '.jpg"></img>' +
        '<p>' + cn_arr_long2[i] + '</p></div>';
    instruction_pages_long2.push(slide);
}

const cn_instructions_long2 = {
    type: jsPsychInstructions,
    ...
    on_finish: function() { 
        var prog = jsPsych.getProgressBarCompleted();
        jsPsych.setProgressBar(prog + 0.5); 
    }
};
