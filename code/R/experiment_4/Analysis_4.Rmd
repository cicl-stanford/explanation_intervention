...
df.exp4 <- df.data4 %>% 
  ...
  mutate(CausalStructure =
           case_when(
       str_detect(index, "cp_") | str_detect(index, "cn_") ~ "conj",
       str_detect(index, "dp_") | str_detect(index, "dn_") ~ "disj"))%>% 
  mutate(Outcome =
           case_when(
  ...
  mutate(ResponseType =
           case_when(
       str_detect(index, "one_") ~ "probability_Normal_Intervened",
       str_detect(index, "two_") ~ "probability_Abnormal_Intervened"))%>% 
  mutate(CausalStructure= factor(CausalStructure, levels=c("conj", "disj"))) %>%
